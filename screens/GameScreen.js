...
export const GameScreen = (props) => {
  ...
  const nextGuessHandler = (direction) => {
    if (isGreaterHintTrue(direction)) {
      showInvalidAlert(
        " You know this is wrong..., the number should be lower"
      );
      return;
    }
    if (isLowerHintTrue(direction)) {
      showInvalidAlert(
        " You know this is wrong..., the number should be higher"
      );
      return;
    }
    direction === "lower"
  ...
  const isGreaterHintTrue = (hint) =>
    hint === "lower" && currentGuess < userChoice;
  const isLowerHintTrue = (hint) =>
